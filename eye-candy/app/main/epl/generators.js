...
function* concatGenerators(...generators) {
  for (const g of generators) {
    for (const stimulus of g) {
      yield stimulus;
    }
  }
}
...
function* measureIntegrity(stimuli, every = 5 * 60) {
  ...
  for (const s of stimuli) {
    if (elapsedTime >= every && s.metadata.block === undefined) {
      integrityMeta = { group: stimuli.uuid(), label: 'integrity' };
...
function createCelltypingGen(r, windowHeight, windowWidth, nRows, nCols) {
  ...
  celltypeStimuli.push(
    new Chirp(8, 0.5, 5, 127.5, 127.5, 8, -PI / 2, celltypeMeta)
  );
  celltypeStimuli.push(new Wait(3, celltypeMeta));
  ...
  // moving bars
  // baden params: 0.3 x 1 mm bright bar moving at 1 mm s−1
  ...
  const ctWidth = 110;
  const ctSpeed = 367;
  const ctAngles = [...Array(32).keys()].map((x) => ((x * 2 + 1) * PI) / 32);
  const ctLifespan = calcBarLifespan(
    ctSpeed,
    ctWidth,
    windowHeight,
    windowWidth
  ...
  celltypeStimuli.push(
    new WhiteNoise(60 * 5, nRows, nCols, false, celltypeMeta)
  );

  function* stimulusGenerator() {
    for (const s of celltypeStimuli) {
      yield s;
    }
  }
